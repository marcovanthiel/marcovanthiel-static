#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Genereert static/italiecamper2026/index.html (campervariant van de
pareltjesroute) uit route.json + template.html. Stijl, foto's, vendor-assets
en weer.json worden HERGEBRUIKT van /italie2026 (absolute paden), dus deze
map bevat alleen route.json + index.html.

Gebruik:  python3 scripts/italiecamper2026/build.py
...
- De GitHub Action italie2026-build.yml draait dit script bij wijziging;
  italie2026-weer.yml draait het dagelijks mee voor de temperatuurregels.
"""
import html
import json
import pathlib
...
SITE = HIER.parents[1] / "static" / "italiecamper2026"
ASSETSITE = HIER.parents[1] / "static" / "italie2026"   # gedeelde assets + weer

# Etappenummers van de twee ankers (visueel onderscheiden):
# 3 = Portico di Romagna (Acquacheta), 4 = Verona (opera, Castel San Pietro)
ANKERS = {3, 4}
...
def etappe_html(e, weer=None):
    ...
    # Reisgids-weergave: één rustige campingregel (naam + link + korte
    # typering van de faciliteiten), zonder prijzen.
    sug = e.get("camping")
    sugblok = ""
    if sug:
        sugblok = (
    ...
    if e["nr"] == 4:
      ...
      <aside class="opera" aria-label="Opera in de Arena di Verona">
        ...
        <p class="waarschuwing"><span class="lang lang-nl" lang="nl">Vanaf de camping wandel je in een kwartier naar de Arena en na afloop weer terug; tickets via arena.it.</span><span class="lang lang-zh" lang="zh">从营地步行一刻钟即到圆形剧场,散场后步行返回;门票请在 arena.it 预订。</span></p>
      </aside>"""
